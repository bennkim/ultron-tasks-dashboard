...
<div class="home-grid">
  <div class="panel"><h3>🚨 블로커 (<span id="home-blocker-count">0</span>건)</h3><div id="home-blockers" style="color:var(--muted);font-size:.85rem">로딩 중...</div></div>
...
function updateHomeStats(tasks){
  ...
  const blockersHtml=blockers.length===0?'<p style="color:var(--muted);font-size:.85rem">없음 🎉</p>':blockers.map(b=>\`<div class="blocker-item"><span class="badge badge-blocked">🚨</span><span class="rank-id">\${b.id}</span><span>\${b.title||b.description}</span><span style="color:var(--muted);margin-left:auto">\${b.owner||'—'}</span></div>\`).join('');
  document.getElementById('home-blockers').innerHTML=blockersHtml;
  
  // ICE Ranking
  const iceRanking=tasks.filter(t=>t.status!=='DONE'&&t.ice_score>0).sort((a,b)=>b.ice_score-a.ice_score).slice(0,10);
  const iceHtml=iceRanking.map((t,i)=>\`<div class="rank-item"><span class="rank-num">\${i+1}</span><span class="rank-id">\${t.id}</span><span>\${(t.title||t.description||'').substring(0,30)}\${(t.title||t.description||'').length>30?'…':''}</span><span class="rank-score"><span class="ice \${iceClass(t.ice_score)}">\${t.ice_score}</span></span></div>\`).join('');
