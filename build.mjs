...
.task-table th:nth-child(4),.task-table td:nth-child(4){width:auto;text-align:left}
.task-table th:nth-child(5),.task-table td:nth-child(5){width:90px;white-space:nowrap;font-size:.75rem;color:var(--muted);text-align:center}
...
.history-modal{background:var(--card);border-radius:16px;padding:24px;max-width:640px;width:90vw;max-height:85vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,.15);position:relative}
.ticket-meta{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px}
.ticket-meta .badge{font-size:.72rem;padding:3px 10px}
.ticket-fields{display:grid;grid-template-columns:1fr 1fr;gap:8px 16px;margin-bottom:18px;padding:14px;background:var(--bg);border-radius:10px;border:1px solid var(--border)}
.ticket-field{font-size:.8rem}.ticket-field-label{color:var(--muted);font-weight:600;font-size:.7rem;text-transform:uppercase;letter-spacing:.3px;margin-bottom:2px}.ticket-field-value{color:var(--text);word-break:break-word}
.ticket-field.full{grid-column:1/-1}
.ticket-section-title{font-size:.82rem;font-weight:700;color:var(--blue-dark);margin-bottom:10px;padding-top:4px;border-top:1px solid var(--border)}
.ticket-commit{font-family:'SF Mono',monospace;font-size:.75rem;color:var(--blue);background:var(--blue-light);padding:2px 8px;border-radius:6px;text-decoration:none}.ticket-commit:hover{text-decoration:underline}
...
<div class="history-modal-overlay" id="historyOverlay" onclick="if(event.target===this)closeHistoryModal()">
  <div class="history-modal ticket-detail">
    <div class="history-modal-header">
      <span class="history-modal-title" id="historyTitle"></span>
      <button class="history-modal-close" onclick="closeHistoryModal()">✕</button>
    </div>
    <div id="ticketMeta" class="ticket-meta"></div>
    <div id="ticketFields" class="ticket-fields"></div>
    <div class="ticket-section-title">📜 활동 로그</div>
...
async function openHistoryModal(id,title){
  ...
  const fieldsEl=document.getElementById('ticketFields');
  const body=document.getElementById('historyBody');
  metaEl.innerHTML='';fieldsEl.innerHTML='';
  ...
  try{
    ...
    if(t){
      // Title update
      document.getElementById('historyTitle').textContent=t.id+' — '+(t.title||t.description||'');
      // Meta badges
      let meta='<span class="badge badge-'+statusClass(t.status)+'">'+statusEmoji(t.status)+' '+t.status+'</span>';
      if(data.epic)meta+='<span class="badge" style="background:var(--blue-light);color:var(--blue)">'+data.epic.id+' '+data.epic.title+'</span>';
      if(data.story)meta+='<span class="badge" style="background:rgba(245,158,11,.1);color:var(--yellow)">'+data.story.id+'</span>';
      if(t.ice_score>0)meta+='<span class="ice '+iceClass(t.ice_score)+'">ICE '+t.ice_score+'</span>';
      metaEl.innerHTML=meta;
      // Fields grid
      const f=(label,val,full)=>'<div class="ticket-field'+(full?' full':'')+'"><div class="ticket-field-label">'+label+'</div><div class="ticket-field-value">'+(val||'—')+'</div></div>';
      let fields='';
      fields+=f('요청자 (Owner)',t.owner);
      fields+=f('작업자 (Assignee)',t.assignee);
      fields+=f('착수일',t.started_at);
      fields+=f('완료일',t.completed_date);
      fields+=f('생성일',t.created_at?.substring(0,16));
      fields+=f('수정일',t.updated_at?.substring(0,16));
      if(t.blocked_reason)fields+=f('🚨 블로커 사유',t.blocked_reason,true);
      if(t.related_commit){
        const commitHtml=t.related_commit.startsWith('http')?'<a class="ticket-commit" href="'+t.related_commit+'" target="_blank">'+t.related_commit+'</a>':'<span class="ticket-commit">'+t.related_commit+'</span>';
        fields+=f('관련 커밋/PR',commitHtml,true);
      }
      if(t.completion_criteria)fields+=f('✅ 완료 조건',t.completion_criteria,true);
      if(t.notes)fields+=f('📝 메모',t.notes,true);
      if(t.description&&t.description!==t.title)fields+=f('상세 설명',t.description,true);
      fieldsEl.innerHTML=fields;
    }else{metaEl.innerHTML='';fieldsEl.innerHTML='';}
    // History timeline
    ...
    metaEl.innerHTML='';fieldsEl.innerHTML='';
    body.innerHTML='<p style="color:var(--red);font-size:.85rem">로드 실패: '+err.message+'</p>';
  }
}
